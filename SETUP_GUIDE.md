...
Version 3.3.2 uses a network-first service-worker strategy, so configuration changes should update normally. If an older installed copy remains stale, close all app windows, reopen the site in Edge, press `Ctrl+F5`, and then reopen the installed app.
